#include "ugv_lidar_preprocessing/utils/imu_integrator.hpp"
                         ...
                         const lidar_preprocessing_plugins::plugins_context_data::ImuSampleBuffer &imu_samples)
{
    ...
    // Since we don't have external odometry (GPS/LIO) to correct the IMU, pure integration
    // would cause 'v' to drift to infinity over time.
    init_state.v = Eigen::Vector3f::Zero();
